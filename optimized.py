...
class DataApp:
    ...
    def show_visualization(self):
        if self.data is not None:
            ...
            if len(selected_two) == 2:
                ...
                ).properties(title=f"Line plot of {y_col} vs {x_col}", width='stretch', height=300)
    ...
    def clean_data(self):
        if self.data is not None:
            ...
            st.dataframe(report_before.fillna("-").astype(str), width='stretch')
            ...
            st.dataframe(report_after.fillna("-").astype(str), width='stretch')
            st.write("Cleaned Data")
            st.dataframe(cleaned_latest, width='stretch')
        ...
        if self.data is not None:
            ...
            for i in range(0, len(num_cols), 2):
                ...
                for holder, col in zip(cols, pair):
                    ...
                    # Ensure column name is a safe string
                    safe_col = f"col_{str(col).replace(' ', '_')}"
                    df_plot = data.to_frame(name=safe_col)
                    chart = alt.Chart(df_plot).mark_bar().encode(
                        x=alt.X(f'{safe_col}:Q', bin=alt.Bin(maxbins=int(bins)), title=str(col)),
                        y=alt.Y('count():Q', title='Count')
                    ).properties(height=280)
                    holder.altair_chart(chart, width='stretch')
